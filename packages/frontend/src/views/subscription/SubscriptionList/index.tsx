...
function stableSort(
	array: TGetSubscription[],
	comparator: (a: TGetSubscription, b: TGetSubscription) => number
) {
	const stabilizedThis = array.map((el: TGetSubscription, index: number) => [
		el,
		index,
	]);
	stabilizedThis.sort((a, b) => {
		const el = comparator(a[0] as TGetSubscription, b[0] as TGetSubscription);
		if (el !== 0) return el;
...
const RoleList = () => {
	...
	const [rows, setRows] = React.useState<TGetSubscription[]>([]);
	const [openCreate, setOpenCreate] = React.useState(false);
	const [openDetails, setOpenDetails] = React.useState(false);
	const [openEdit, setOpenEdit] = React.useState(false);
	const [rowData, setRowData] = React.useState<TGetSubscription | undefined>(
		undefined
	);
	...
	const handleOpenDetails = (row: TGetSubscription) => () => {
		setRowData(row);
		setOpenDetails(true);
	};
	...
	const handleOpenEdit = (row: TGetSubscription) => () => {
		setRowData(row);
		setOpenEdit(true);
	};
	...
	const handleDelete = (selectedItemsArray: number[]) => {
		selectedItemsArray.forEach((item) => {
			dispatch(deleteSubscription(item, { sync: true }));
			setSelected([]);
		});
	};
	...
	return (
		<MainCard title="Subscription List" content={false}>
			<CardContent>
				<Grid
					container
			...
			<TableContainer>
				...
				{openCreate && (
					<SubscriptionAdd handleCloseDialog={handleCloseDialog} />
				)}
			</TableContainer>
